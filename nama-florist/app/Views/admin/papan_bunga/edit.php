...
<div class="flex justify-between items-center mb-6">
    <h1 class="font-display text-2xl md:text-3xl font-bold text-primary">Edit Produk: <?= esc($product['nama']) ?></h1>
...
<div class="rounded-xl border border-gray-200 dark:border-gray-800 bg-background-light dark:bg-background-dark/50 p-6">
    <!-- PENTING: Arahkan form ke rute update -->
    <?= form_open_multipart('admin/papanbunga/' . $product['id']) ?>
    <!-- PENTING: Selipkan input ini untuk method spoofing -->
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div class="space-y-4">
            ...
            <div>
                ...
                <input type="text" name="bahan" id="bahan" value="<?= old('bahan', $product['bahan']) ?>"
                    class="form-input w-full rounded-lg bg-primary/10 border-none focus:ring-2 focus:ring-primary/50">
            </div>
            <div>
            ...
            <div>
                ...
                <select name="status" id="status"
                    ...
                    <option value="tersedia" <?= old('status', $product['status']) == 'tersedia' ? 'selected' : '' ?>>
                        Tersedia</option>
                    <option value="tidak_tersedia" <?= old('status', $product['status']) == 'tidak_tersedia' ? 'selected' : '' ?>>Tidak Tersedia</option>
                </select>
            </div>
            <div>
                <label class="block text-sm font-medium mb-1">Ganti Gambar Produk (Opsional)</label>
                <div class="flex items-center gap-4">
                    <img src="<?= base_url('uploads/images/' . $product['gambar']) ?>"
                        alt="<?= esc($product['nama']) ?>" class="h-16 w-20 object-cover rounded-md">
                    <input type="file" name="gambar" id="gambar" class="form-input w-full">
                </div>
                <p class="mt-1 text-xs text-gray-500">Kosongkan jika tidak ingin mengganti gambar. PNG, JPG, JPEG. Maks
                    2MB.</p>
            </div>
        </div>
    </div>
